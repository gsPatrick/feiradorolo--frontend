...
export const productService = {
  ...
  // Autocomplete de busca (público) → [{ term }].
  suggestions: (q) => api.get('/products/suggestions?q=' + encodeURIComponent(q || ''), { auth: false }),
...
export function mapProduct(p) {
  ...
  return {
    ...
    sellerId: (p.seller && p.seller.id) || p.seller_id || null,
    allowPickup: !!(p.metadata && (p.metadata.allow_pickup || p.metadata.pickup_available)),
  };
}
...
/* Presença/visitantes anônimos. Ping público — não exige login. */
export const presenceService = {
  // data: { session_id, path }
  ping: (data) => api.post('/presence/ping', data, { auth: false }),
};
...
export const adminService = {
  ...
  // Verificações de identidade (KYC) — listar pendentes + revisar (aprovar/rejeitar).
  // Backend: GET /users/verifications?status=pending (authorize users.verify).
  verifications: (params = '') => api.get('/users/verifications' + params),
  reviewVerification: (id, data) => api.patch('/users/verification/' + id + '/review', data),
  // Anúncios (admin) — controle total de QUALQUER produto (qualquer status/vendedor).
  products: (params = '') => api.get(`/products/admin/all${params}`),
  setProductStatus: (id, status) => api.patch(`/products/${id}/status`, { status }),
  deleteProduct: (id) => api.del(`/products/${id}`),
  // Presenteia um destaque sem pagamento. data: { tier, days } (tier silver|gold|diamond|none).
  setProductBoost: (id, data) => api.post(`/products/${id}/admin-highlight`, data),
  // Moderação de contas
  approveUser: (id) => api.post(`/users/${id}/approve`),
  suspendUser: (id, data) => api.post(`/users/${id}/suspend`, data),
  banUser: (id, data) => api.post(`/users/${id}/ban`, data),
  unbanUser: (id) => api.post(`/users/${id}/unban`),
  setChatOnly: (id) => api.post(`/users/${id}/chat-only`),
  removeChatOnly: (id) => api.post(`/users/${id}/chat-only/remove`),
  deleteUser: (id) => api.del(`/users/${id}`),
  // Ações em massa. data: { ids:[...], action, payload? } → { ok, failed }.
  bulkProducts: (data) => api.post('/products/admin/bulk', data),
  bulkUsers: (data) => api.post('/users/admin/bulk', data),
  ...
  // Tráfego/presença: { online_now, online_users, visitors_today/week/month, page_hits_today, series }
  ...
  // Concessão manual de plano a um usuário (grant sem pagamento).
  // data: { user_id, plan_id, days? } → cria assinatura ATIVA.
  grantPlan: (data) => api.post('/plans/admin/grant', data),
  // Assinaturas de um usuário (status, plan, ends_at).
  userSubscriptions: (userId) => api.get(`/plans/admin/subscriptions?user_id=${userId}`),
  // Revoga (cancela) uma assinatura.
  revokeSubscription: (id) => api.del(`/plans/admin/subscriptions/${id}`),
};
...
export const verificationService = {
  ...
  // Gera uma sessão de biometria facial (QR no celular) → { url, expires_at }.
  facialSession: () => api.post('/users/me/verification/facial-session'),
  // Envia o pacote de verificação de vendedor (KYC) para análise.
  submitSeller: (data) => api.post('/users/me/verification', data),
};

/* Tabela FIPE (público — não exige login). Cascata tipo→marca→modelo→ano→valor.
   marcas/modelos/anos → [{ codigo, nome }].
   valor → { valor, marca, modelo, anoModelo, combustivel, codigoFipe, mesReferencia }. */
export const fipeService = {
  marcas: (tipo = 'carros') => api.get('/fipe/marcas?tipo=' + tipo, { auth: false }),
  modelos: (tipo, marca) => api.get('/fipe/modelos?tipo=' + tipo + '&marca=' + marca, { auth: false }),
  anos: (tipo, marca, modelo) =>
    api.get('/fipe/anos?tipo=' + tipo + '&marca=' + marca + '&modelo=' + modelo, { auth: false }),
  valor: (tipo, marca, modelo, ano) =>
    api.get('/fipe/valor?tipo=' + tipo + '&marca=' + marca + '&modelo=' + modelo + '&ano=' + ano, { auth: false }),
};
