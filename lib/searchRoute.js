...
const VERTICALS = [
  ...
  // Imóveis (quando a vertical existir):
  // { test: /(^|\s)im[oó]ve(l|is)(\s|$)/i, route: (q) => `/imoveis${q ? `?q=${encodeURIComponent(q)}` : ''}` },
...
export function categoryRoute(slug) {
  ...
  // Imóveis / Veículos (quando as verticais existirem):
  // if (/im[oó]ve/i.test(s)) return '/imoveis';
