...
const VERTICALS = [
  ...
  // Veículos
  { test: /(^|\s)(ve[ií]culos?|carros?|motos?|caminh[oõ]es?|[oô]nibus|autom[oó]ve(l|is))(\s|$)/i, route: (q) => `/veiculos/lista${q ? `?q=${encodeURIComponent(q)}` : ''}` },
];
...
export function categoryRoute(slug) {
  ...
  if (/^veiculos$|^veiculo|^carros|^motos|^caminhoes|^onibus/i.test(s)) return '/veiculos';
  return `/categoria/${s}`;
}
